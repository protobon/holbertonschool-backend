...
class MRUCache(BaseCaching):
    """Cache with Most Recently Used algorithm"""
    use_count = {}
    ...
    def put(self, key, item):
        ...
        set, then the key that's been used
        the most gets removed (MRU).
        """
        if not key or not item:
            return
        if key not in self.cache_data.keys():
            if len(self.cache_data) == BaseCaching.MAX_ITEMS:
                del_key = list(self.use_count.keys())[-1]
    ...
    def get(self, key):
        ...
        of Most Recently Used (to the first place).
        """
